...
import copy
import torch
from preset import resl_to_batch, resl_to_lr, resl_to_num
...
class PGGANrunner:
    def __init__(self, arg, G, D, scalable_loader, torch_device, loss, tensorboard):
        ...
        self.tran_step = self.img_num // self.batch   ### batch_num
        self.stab_step = self.img_num // self.batch
        print("init tran_step=%s, stab_step=%s: " %(self.tran_step, self.stab_step))
        self.G = G
        ...
        if loss == "lsgan":
            self.step = Train_LSGAN(self.G, self.D, self.optim_G, self.optim_D,
                                    self.arg.label_smoothing, self.batch, self.device)
        elif loss == "wgangp":
            self.step = Train_WGAN_GP(self.G, self.D, self.optim_G, self.optim_D,
                                      self.arg.gp_lambda, self.arg.eps_drift ,self.batch, self.device)
    ...
    def grow_architecture(self, resl, global_step):
        ...
        self.stab_step = (self.img_num // self.batch) * resl_to_num[resl]
        self.tran_step = (self.img_num // self.batch) * resl_to_num[resl]
        print("current stage tran_step=%s, stab_step=%s: " %(self.tran_step, self.stab_step))
